...
const createPremiumOrder = async (req, res) => {
  ...
  try {
    ...
    // -----------------------------------------------
    // PREVENT BUYING AGAIN WHILE ACTIVE
    // -----------------------------------------------
...
const createPremiumPlusOrder = async (req, res) => {
  try {
    const user = await User.findById(
      req.user.userId
    );
    ...
    // Allow Premium users to upgrade to Premium Plus.
    // Prevent another Premium Plus purchase while
    // the current Premium Plus plan is still active.
    if (
      user.plan === "premium_plus" &&
      user.planExpiresAt &&
      new Date(user.planExpiresAt) > new Date()
...
const verifyPremiumPayment = async (
  ...
  try {
    ...
    // -----------------------------------------------
    // CHECK PAYMENT BELONGS TO PREMIUM
    // -----------------------------------------------
...
const verifyPremiumPlusPayment = async (
  req,
  res
) => {
